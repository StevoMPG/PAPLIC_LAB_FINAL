...
public class manejadorLogs implements ILogger {
	@PersistenceUnit(name="LoggerDB")
	EntityManagerFactory f = Persistence.createEntityManagerFactory("LoggerDB");
	private static manejadorLogs instance = null;
	
	private manejadorLogs() { }
	
	public static ILogger getInstance() {
		if (instance == null)
			instance = new manejadorLogs(); 
		return instance;
	}
	...
	public LogEntryWS [] getLogs() {
		...
		try {
			...
			for(LogEntry ent :  q.getResultList()) {
				...
				rq.setDate(new DtFechaWS(new DtFechaHora(ent.getDate().get(Calendar.YEAR),ent.getDate().get(Calendar.MONTH)+1,
