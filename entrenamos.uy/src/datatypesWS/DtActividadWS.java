...
public class DtActividadWS implements Serializable {
	...
	public DtActividadWS(DtActividadDeportivaExtra d) {
	...
	public DtActividadDeportivaExtra adapt() {
		if(categorias==null)
		...
		if(cup==null)
			cup = new String[0];
		return new DtActividadDeportivaExtra(nombre,descripcion,duracionMinutos,costo,fechaRegistro.adapt(),
		new HashSet<>(Arrays.asList(categorias)),new HashSet<>(Arrays.asList(clases)),new HashSet<>(Arrays.asList(cup)),tipoEstado.values()[estado.ordinal()],creador,imgName,favoritos);
	}
