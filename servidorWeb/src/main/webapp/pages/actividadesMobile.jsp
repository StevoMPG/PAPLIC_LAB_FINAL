...
<%  DtActividadDeportivaExtra datosActDep = (DtActividadDeportivaExtra) request.getAttribute("actDep");
  ...
  <div class="row pt-4 pb-4" id="section-clases">
      ...
      <a href="<%=request.getContextPath()%>/clases?clase=<%=((DtClaseExtra)dtClase).getNombre()%>">
          <img alt="imgClase"  src="<%=request.getContextPath()%>/api/content?c=cla&id=<%=((DtClaseExtra)dtClase).getNombre()%>" class="vertical-align-middle imgSelectMobile">
          <div class="row">
              <h5><%=((DtClaseExtra)dtClase).getNombre()%></h5>
          </div>
      </a>
      <%contador++;
