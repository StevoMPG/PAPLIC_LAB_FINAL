    ...
    <div id="section-verActsDeps" class="row pt-4 pb-4">
        ...
        <%if ((((Set<?>)request.getAttribute("filtroInsti")).size() > 0) || (((Set<?>)request.getAttribute("filtroCat")).size() > 0)) {%>
          <div class="btn-group">
            ...
            <%for (Object obj : listaActividades) {%>
              <li><a class="dropdown-item" href="<%=linkAndFiltro%>&mostrar=<%=mostrarTodas%>&fltrA=<%=((DtActividadDeportivaExtra)obj).getNombre()%>"><%=((DtActividadDeportivaExtra)obj).getNombre()%></a></li>
            <% } %>
            </ul>
          </div>
        <% } %>
        ...
        <div id="actsDeps" class="row">
        	<%if (encabezado.equals("Clases")) {
	            ...
	            <a href="<%=request.getContextPath()%>/clases?clase=<%=((DtClaseExtra)obj).getNombre()%>">
	            <div class="row pt-2">
	                <img class="imgSearchMobile" src="<%=request.getContextPath()%>/api/content?c=cla&id=<%=((DtClaseExtra)obj).getNombre()%>" alt="<%=((DtClaseExtra)obj).getNombre()%>">
	            </div>
		        <div class="row pt-2 pb-2">
	                <h3><%=((DtClaseExtra)obj).getNombre()%></h3>
	            </div>
	            </a>
            	<% } %>
            <% } else { 
            	for (Object obj : listaActividades) {%>
   	            <a href="<%=request.getContextPath()%>/actividades?actividad=<%=((DtActividadDeportivaExtra)obj).getNombre()%>">
   	            <div class="row pt-2">
   	                <img class="imgSearchMobile" src="<%=request.getContextPath()%>/api/content?c=act&id=<%=((DtActividadDeportivaExtra)obj).getNombre()%>" alt="<%=((DtActividadDeportivaExtra)obj).getNombre()%>">
   	            </div>
   		        <div class="row pt-2 pb-2">
   	                <h3><%=((DtActividadDeportivaExtra)obj).getNombre()%></h3>
   	            </div>
   	            </a>
               	<% } %>
            <% } %>
