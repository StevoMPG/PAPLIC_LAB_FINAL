...
public class Search extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request,  
    	...
    	if ((clase != null) && (clase.equals("yes"))) {
    		try {
    			boolean seMuestra = (mostrar.equals("no")) ? false : true;
        		listaClases = obtenerClases(fltrInstituciones, filtroAct, seMuestra);
        		switch(orden) {
	    			case "alfaDesc":
    	...
    	request.setAttribute("mostrarTodas", mostrar);
			...
			if (fltrI.isEmpty() || fltrI.contains(x)) {
				try {
					for (String y : IADC.obtenerActividades(x)) {
						DtActividadDeportiva datosActividad = IADC.getActDepExtra(x,  y);
						if (((datosActividad.getNombre().contains(texto)) || (datosActividad.getDescripcion().contains(texto))) && 
								(datosActividad.getEstado() == tipoEstado.aceptada) && ((fltrC == null) || 
	...
	private List<DtClase> obtenerClases(Set<String> fltrI, String fltrA, boolean mostrar) throws ClaseException {
			...
			if (fltrI.isEmpty() || fltrI.contains(x)) {
				try {
					for (String y : IDCC.obtenerActividades(x)) {
						if (fltrA.isEmpty() || (fltrA.equals(y) && (IADC.getActDepExtra(x, y).getEstado() == tipoEstado.aceptada))) {
			...
			if (cup.getNombre().contains(texto) || cup.getDescripcion().contains(texto)) {
				if (fltrC.isEmpty() || cup.getCategorias().containsAll(fltrC)) {
					if (fltrI.isEmpty()) {
						...
						List<DtClasesCuponeras> clCup = cup.getContenido();
						for (DtClasesCuponeras y : clCup) {
							boolean freno = false;
							for (String z : fltrI) {
								try {
