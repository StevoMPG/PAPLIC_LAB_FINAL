...
public class Actividades extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request,  HttpServletResponse response) throws ServletException,  IOException {
    	...
    	DtUsuarioExtra nickUser = (DtUsuarioExtra) request.getSession().getAttribute("loggedUser");
		...
		if (datosActDep != null) {
			...
			if (!finalizada) {
				...
				for(String x: datosActDep.getClases()) {
					try {
						DtClaseExtra cl = LaFabricaWS.getInstance().obtenerIActDeportivaController().seleccionarClase(institucion, datosActDep.getNombre(), x);
						f1 = cl.getFechaClase().esMenor(new DtFechaHora());
		...
		if (nickUser != null && nickUser instanceof DtSocioExtra) {
			esSocio = true;
		}
	...
	private DtActividadDeportivaExtra buscarActDep(String nombreActDep) throws ActividadDeportivaException {
    	DtActividadDeportivaExtra datosActDep = LaFabricaWS.getInstance().obtenerIActDeportivaController().buscarActDep(nombreActDep);
    	return datosActDep;
    }
}
